...
SearchResult NegaScout(Position& position, unsigned int initialDepth, int depthRemaining, int alpha, int beta, int colour, unsigned int distanceFromRoot, bool allowedNull, SearchData& locals, ThreadSharedData& sharedData)
{
	...
	if (!IsPV(beta, alpha)) 
	{
		std::optional<TTEntry> entry = tTable.GetEntry(position.GetZobristKey(), distanceFromRoot, position.GetTurnCount(), depthRemaining);

		//Don't take scores from the TT if there's a two-fold repitition
		if (entry && !position.CheckForRep(distanceFromRoot, 2) && UseTransposition(*entry, alpha, beta))
